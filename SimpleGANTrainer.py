    ...
    def train(self, n_epochs, n_batch, do_wass_viz=True):
        ...
        for epoch in range(n_epochs):
            ...
            if do_wass_viz:
                ...
                fake_data, _ = self.in_functions["G"](n_batch)
                # Obtain batch of real data
                real_data = self.dataset(n_batch, self.device)
                # Find and record the Wasserstein Distance
                was_dist = self.all_Wasserstein_dists(fake_data, real_data).mean()
                self.stats["W_Dist"].append(was_dist.item())
